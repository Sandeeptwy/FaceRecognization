...
def index_faces(bucket, key):
    response = rekognition.index_faces(
        ...
        CollectionId="cricketer"
    )
    return response
    
def update_index(tableName, faceId, userName, fullName, email):
...
def lambda_handler(event, context):
    try:
        ...
        if response['ResponseMetadata']['HTTPStatusCode'] == 200:
            ...
            person_user_name = object_metadata['Metadata'].get('username', 'Unknown')
            person_full_name = object_metadata['Metadata'].get('fullname', 'Unknown')
            person_email = object_metadata['Metadata'].get('email', 'Unknown')

            update_index('cricketers_collection', faceId, person_user_name, person_full_name, person_email)

            print("Face indexed successfully. FaceId: {}, Full Name: {}".format(faceId, person_user_name, person_full_name, person_email))
